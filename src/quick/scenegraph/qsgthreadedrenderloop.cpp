    ...
    if (!w->thread->isRunning()) {
        ...
        if (!w->thread->gl) {
            ...
            if (!w->thread->gl->create()) {
                delete w->thread->gl;
                w->thread->gl = 0;
                qCDebug(QSG_LOG_RENDERLOOP) << "- failed to create OpenGL context";
                return;
            }
...
void QSGThreadedRenderLoop::releaseResources(Window *w, bool inDestructor)
{
    ...
    if (w->thread->isRunning() && w->thread->active) {
        ...
        // can exit properly.
        if (!w->thread->active) {
            qCDebug(QSG_LOG_RENDERLOOP) << " - waiting for render thread to exit";
            w->thread->wait();
            qCDebug(QSG_LOG_RENDERLOOP) << " - render thread finished";
        }
    }
    w->thread->mutex.unlock();
}
