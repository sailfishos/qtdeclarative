...
void QSGContext::renderContextInitialized(QSGRenderContext *renderContext)
{
    ...
    if (!dumped && QSG_LOG_INFO().isDebugEnabled()) {
        ...
        qCDebug(QSG_LOG_INFO) << "GL_VENDOR:         " << (const char *) glGetString(GL_VENDOR);
        qCDebug(QSG_LOG_INFO) << "GL_RENDERER:       " << (const char *) glGetString(GL_RENDERER);
        qCDebug(QSG_LOG_INFO) << "GL_VERSION:        " << (const char *) glGetString(GL_VERSION);
